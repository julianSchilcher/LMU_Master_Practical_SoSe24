...
class TestMinibatchKmeans(unittest.TestCase):
    ...
    def sample_cluster_tree_with_assignments(self):
        """
        Helper method for creating a sample cluster tree with assignments
        """
        tree = self.sample_cluster_tree()
        tree.assign_to_nodes(
            torch.tensor([[-3, -3], [10, 10], [-0.4, -0.4], [0.4, 0.3]])
        )
        return tree
    ...
    def test_cluster_tree_assignment(self):
        tree = self.sample_cluster_tree_with_assignments()

        # check if all assignments made correct
        assert torch.all(
            torch.eq(
                tree.root.left_child.left_child.assignments, torch.tensor([[-3, -3]])
            )
        )
        assert torch.all(
            torch.eq(
