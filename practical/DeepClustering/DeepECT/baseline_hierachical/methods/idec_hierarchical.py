...
def run_experiment(data, ground_truth_labels, new_seed, n_clusters, autoencoder, device="cpu"):
    ...
    for batch_data in torch.utils.data.DataLoader(pt_init_sample, batch_size=256, shuffle=True):
        ...
        embedded_batch_np = autoencoder.encode(batch_data.to(device)).detach().cpu().numpy()
    ...
    optimizer = torch.optim.Adam(list(autoencoder.parameters()) + list(cluster_module.parameters()), lr=0.001)
    ...
    def evaluate(train_round_idx, ae_module, cluster_module):
        ...
        return dp_value_single, dp_value_complete, leaf_purity_value_single, leaf_purity_value_complete
    ...
    dp_value_single, dp_value_complete, leaf_purity_value_single, leaf_purity_value_complete = evaluate("", autoencoder, cluster_module)
    return dp_value_single, dp_value_complete, leaf_purity_value_single, leaf_purity_value_complete
